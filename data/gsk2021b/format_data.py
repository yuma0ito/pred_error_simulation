...
def format_data(theme, response_path, score_path):
    ...
    for key in score_dict:
        mean_score_dict[key] = [
            mean(score_per_item) for score_per_item in score_dict[key]
        ]

    response_dict = {key: response_dict[key] for key in sorted(response_dict)}
    mean_score_dict = {key: mean_score_dict[key] for key in response_dict}
    assert response_dict.keys() == mean_score_dict.keys()
    ...
    df_score = pd.DataFrame(mean_score_dict).T
    ...
    df_score[item_columns] = (df_score[item_columns] + 0.5).astype(int)
